    ...
    for k=1:Ntest
        ...
        for i=1:Nalgo
            ...
            for j=1:Nrun
                ...
                % Get reference front
                [RefPareto{k},RefFPareto{k},RefGPareto{k},A]=ref_front_sorting(rst(j).ppareto{end}(:,fea_ind),rst(j).fpareto{end}(:,fea_ind),rst(j).gpareto{end}(:,fea_ind),RefPareto{k},RefFPareto{k},RefGPareto{k},A,500);
            end
%             figure(1);hold on;
%             if i>1
%                 set(obj,'color','k');
    ...
    for k=1:Ntest
        for i=1:Nalgo
            ...
            for j=1:Nrun
                ...
                if flag_conv==1
                    Niter=numel(rst(j).gpareto);
                    for iter=1:Niter
                        ppareto=rst(j).ppareto{iter};
                        fpareto=rst(j).fpareto{iter};
                        gpareto=rst(j).gpareto{iter};
                        [ppareto,fpareto,gpareto]=resortp(ppareto,fpareto,gpareto); % extract only non-dominated feasible solution
                ...
                result.hv(k,i,j) = hypervolume(fpareto,RefPoint{k});
                result.gd(k,i,j) = generational_distance(fpareto,RefFPareto{k});
                result.igd(k,i,j) = inverted_generational_distance(fpareto,RefFPareto{k});
                result.ste(k,i,j) = spacing_to_extent(fpareto);
            end
...
for m=1%:4
    ...
    for k=1:Ntest
        ...
        for i=1:Nalgo
            filename=['rst_' num2str(k,'%03.f') '_' num2str(i,'%03.f') '.mat'];
            load(filename);
            Neval=rst.Neval_History;
            rs_ki=reshape(mean(rs(k,i,:,:),3),1,Niter,1,1);
            rs_conv(i,:)=rs_ki;
            plot(Neval,rs_conv(i,1:numel(Neval)),mc{i});
        end
        lo=legend({'MOBA','MOCRY','MOCS','MOFA','MOFPA','MOHS','MOJS','MOLA'}',...
                'Location','southeast','NumColumns',8,...
