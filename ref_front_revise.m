...
recal = 0; % recal =1 for re-calculate all metric results
    ...
    for k=1:Ntest
        ...
        for i=1:Nalgo
            ...
            for j=1:Nrun
                ...
                % Get reference front
                [RefPareto{k},RefFPareto{k},RefGPareto{k},A]=ref_front_sorting(rst(j).ppareto{end}(:,fea_ind),rst(j).fpareto{end}(:,fea_ind),rst(j).gpareto{end}(:,fea_ind),RefPareto{k},RefFPareto{k},RefGPareto{k},A,Narchive);
            end
%             figure(1);hold on;
%             if i>1
%                 set(obj,'color','k');
